...
function setupRW() {
	...
	for (let i = 0; i < g_arr_ab_3.length; i++) {
		if (g_arr_ab_3[i].length > 0xff) {
			...
			debug_log("[+] Succesfully got a relative R/W", 'green');
	...
	debug_log("[+] Setting up arbitrary R/W", 'orange');
	...
	debug_log("[+] Testing arbitrary R/W", 'orange');
	...
	debug_log("[+] Succesfully got addrof/fakeobj", 'green');
	
	//debug_log("[+] PS4 Jailbreak !", 'green');

	debug_log("[+] PLEASE WAIT !", 'red');
...
function confuseTargetObjRound2() {
	...
	setTimeout(setupRW, 5000);
}


/* Executed after deleteBubbleTree */
function leakJSC() {
	debug_log("[+] Looking for the smashed StringImpl...", 'orange');
	...
	for (let i = arr_str.length - 1; i > 0; i--) {
		if (arr_str[i].length > 0xff) {
			debug_log("[+] StringImpl corrupted successfully", 'green');
	...
	debug_log("[+] Got a relative read", 'green');
	...
	debug_log("[+] JSArrayBufferView: " + g_jsview_leak, 'orange');
...
function dumpTargetObj() {
	changeProgress(25);
	debug_log("[+] m_timer: " + g_timer_leak, 'orange');
	debug_log("[+] m_messageHeading: " + g_message_heading_leak, 'orange');
	debug_log("[+] m_messageBody: " + g_message_body_leak, "orange");
}
	...
	for (let i = 0; i < g_arr_ab_1.length; i++) {
		if (!Int64.fromDouble(g_arr_ab_1[i][2]).equals(Int64.Zero)) {
			...
			if (g_round === 2) {
				...
				g_round++;
				debug_log("[+] Found fake ValidationMessage", 'green');
				changeProgress(5);
			}
...
function sprayHTMLTextArea() {
	...
	element.style.cssText = 'display:block-inline;height:1px;width:1px;visibility:hidden;z-index:-1;position:absolute;';
